...
var onShutdown = function () {
  logger.info('onShutdown() called. Use setShutdownHandler(<function>) to define a specific task...');
};
...
process.on('SIGINT', function () {
  logger.fatal('"'+probe.name+'" process KILLED !!!');
...
process.on('exit', function(code) {
  onShutdown();
  logger.info('"'+probe.name+'" service shuting down gracefully...');
});
